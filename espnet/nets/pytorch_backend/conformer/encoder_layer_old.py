...
class EncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if self.dual_ffn:
            ...
            self.feed_forward_en = feed_forward
        else:
        ...
        if self.conv_module is not None:
            ...
            if self.dual_ffn:
                ...
                self.norm_ff_cn = LayerNorm(size)
                self.norm_final_en = LayerNorm(size)
                self.norm_ff_en = LayerNorm(size)
            else:
    ...
    def forward(self, x_input, mask, ffn_out_input=None, cache=None):
        ...
        if self.dual_ffn:
            ...
            x_cn = residual + stoch_layer_coeff * self.ff_scale * self.dropout(
                self.feed_forward_cn(x)
            )
            x_en = residual + stoch_layer_coeff * self.ff_scale * self.dropout(
                self.feed_forward_en(x)
            )
            if not self.normalize_before:
                x_cn = self.norm_ff_cn(x_cn)
                x_en = self.norm_ff_en(x_en)
        else:
            residual = x
            if self.normalize_before:
                x = self.norm_ff(x)
