...
def initialize(model: torch.nn.Module, init: str):
    ...
    if init == "chainer":
        ...
        if getattr(model, "frontend_de", None):
            if getattr(model.frontend_de, "reload_pretrained_parameters", None):
                model.frontend_de.reload_pretrained_parameters()
            elif isinstance(
                getattr(model.frontend_de, "frontends", None),
                torch.nn.ModuleList,
            ):
                for i, _ in enumerate(getattr(model.frontend_de, "frontends")):
                    if getattr(
                        model.frontend_de.frontends[i],
                        "reload_pretrained_parameters",
                        None,
                    ):
                        model.frontend_de.frontends[i].reload_pretrained_parameters()
        ...
        if getattr(model, "decoder_de", None) and getattr(
            model.decoder_de, "reload_pretrained_parameters", None
        ):
            model.decoder_de.reload_pretrained_parameters()
