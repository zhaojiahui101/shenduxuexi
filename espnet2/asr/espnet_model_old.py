...
class ESPnetASRModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        self.frontend_de = frontend
        self.specaug_de = specaug
        self.normalize_de = normalize
    ...
    def encode(
        ...
        with autocast(False):
            ...
            if self.specaug_de is not None and self.training:
                feats, feats_lengths = self.specaug_de(feats, feats_lengths)

            # 3. Normalization for feature: e.g. Global-CMVN, Utterance-CMVN
            if self.normalize_de is not None:
                feats, feats_lengths = self.normalize_de(feats, feats_lengths)
        ...
        if self.encoder.interctc_use_conditioning:
            ...
            encoder_out, encoder_out_lens, _ = self.encoder(feats, feats_lengths, ctc=self.ctc_de)
        intermediate_outs = None
        if isinstance(encoder_out, tuple):
            intermediate_outs = encoder_out[1]
            encoder_out = encoder_out[0]
    ...
    def _extract_feats(
        ...
        if self.frontend_de is not None:
            ...
            feats, feats_lengths = self.frontend_de(speech, speech_lengths)
        else:
